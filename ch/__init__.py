...
#       * No more joinThread nonsense, Fixes join_room to returns Room Object again
...
from typing import Any, Generator, Protocol, Callable, Optional
import typing
import enum
...
# improve `_get_server` performance by 10x
...
def get_server(group: str) -> str:
    ...
    """
    return "s" + str(specials.get(group, _get_server(group))) + ".chatango.com"
...
class User:
    ...
    _users: dict[str, Any] = dict()

    def __new__(cls, name: str, **_kw: ...) -> Any:
...
class Message:
    ...
    @property
    def uid(self):
        return self.puid  # other library use uid so we create an alias
...
class Task:
    ...
    def _yield_tasks(now: float) -> Generator[Task, None, None]:
        ...
        while Task._tasks_queue and (Task._tasks_queue[0][0] <= now or Task._tasks_queue[0][2].cancelled):
...
class PM:
    ...
    def _getAuth(self, name: str, password: str) -> str | None:
        ...
        data = urllib.parse.urlencode(
            {"user_id": name, "password": password, "storecookie": "on", "checkerrors": "yes"}
        ).encode()
...
class Room:
    ...
    def __init__(self, room: str, uid: str | None, mgr: RoomManager):
        ...
        # if not, we should optimize for better on_message Performance
    ...
    def getUserlist(
        self, mode: Optional[Userlist_Mode] = None, unique: Optional[bool] = None, memory: Optional[int] = None
    ):
        ul = []
    ...
    @property
    def unbanlist(self):
        return [[r.target, r.src] for r in self._unbanlist.values()]
    ...
    def _rcmd_denied(self, _args: list[str]):
        self._disconnect()
        self._callEvent("on_connectFail")
    ...
    def message(self, msg: str, html: bool = False):
        ...
        if not self._bot_name.startswith("!anon"):
            font_properties = '<f x%s%s="%s">' % (self.user.fontSize.zfill(2), self.user.fontColor, self.user.fontFace)
...
class RoomManager:
    ...
    # Init
    ####
    def __init__(self, name: Optional[str] = None, password: Optional[str] = None, pm: bool = True):
        self._name = name
    ...
    # Join/leave
    ####
    def join_room(self, room: str, uid: Optional[str] = None) -> Room:
        """
    ...
    def get_room(self, room: str) -> Room | None:
        """
    ...
    ####
    def _getUser(self):
        return User("@self") if self._name is None else User(self._name)
    ...
    roomnames = property(_get_roomNames)
    pm = property(_getPM)
    ...
    def on_messageDelete(self, room: Room, user: User, message: Message):
        """
    ...
    def on_raw(self, room: Room, raw: str):
        """
    ...
    # Deferring
    ####
    def deferToThread(self, cb: Callable[..., None], func: Callable[..., None], *args: ..., **kw: ...):
        """
    ...
    def setTimeout(self, timeout: int, func: Callable[..., None], *args: ..., **kw: ...) -> Task:
        ...
        if timeout == 0 and Task.running_task is not None and Task.running_task.func == func:
            print(
                "[task][warning] `timeout == 0` will result in high cpu usage with "
            ...
            print(
                "[task][note] An error will be raised to avoid this message being printed "
            ...
            raise RuntimeError("Preemptively exiting to prevent possible log flood causing " "disk space exhaustion")

        task = Task(mgr=self, timeout=timeout, func=func, isInterval=False, args=args, kw=kw)
        return task
    ...
    def setInterval(self, timeout: int, func: Callable[..., None], *args: Any, **kw: Any) -> Task:
        ...
        if timeout == 0:
            print(
                "[RoomManager][setInterval] `timeout == 0` will result in high cpu usage "
                'with "interval usage", use -1 timeout if intended to run once per tick, '
                "or a reasonable amount of timeout like 0.2 (5 times per second)"
            )

        task = Task(mgr=self, timeout=timeout, func=func, isInterval=True, args=args, kw=kw)
        return task
        ...
        while self._running:
            ...
            if not conns:
                if time_to_next_task is None:
                    # Backward compatibilty in case of deferToThread join_room
